...
internal class IconCacheManagerImpl @Inject constructor(
    ...
    // ✅ FIX: ConcurrentHashMap is thread-safe; multiple coroutines can read/write safely
    private val iconCache = ConcurrentHashMap<String, Drawable>()
    private val iconPackCache = ConcurrentHashMap<String, IconPackXmlParser>()

    override fun clearCache() {
        iconCache.clear()
    }

    override fun iconPackFor(packageName: String): IconPackXmlParser =
        iconPackCache.getOrPut(key = packageName) {
    ...
    override fun iconFor(appWithComponent: AppWithComponent, iconPackType: IconPackType): Drawable =
        when (iconPackType) {
            is IconPackType.Custom -> getCustomTypeIcon(
                iconPackPackageName = iconPackType.packageName,
                appWithComponent = appWithComponent
            )
            IconPackType.System -> getSystemTypeIcon(appWithComponent = appWithComponent)
        }

    private fun getSystemTypeIcon(appWithComponent: AppWithComponent): Drawable =
        iconCache.getOrPut(key = appWithComponent.app.packageName) {
            if (appWithComponent.app.packageName == GoogleCalendarIcon.GOOGLE_CALENDAR_PACKAGE_NAME) {
                val drawable = GoogleCalendarIcon.getDrawable(
                    context = context,
                    activityName = appWithComponent.componentName.className
                )
                if (drawable != null) return@getOrPut drawable
            }
            // ✅ FIX: Catch NameNotFoundException for apps uninstalled mid-load
            try {
    ...
    private fun getCustomTypeIcon(
        iconPackPackageName: String,
        appWithComponent: AppWithComponent
    ): Drawable = iconCache.getOrPut(key = appWithComponent.app.packageName) {
        val componentName =
            context.packageManager.getLaunchIntentForPackage(appWithComponent.app.packageName)
                ?.component
        val key = componentName?.toString() ?: appWithComponent.app.packageName
        val iconPack = iconPackFor(packageName = iconPackPackageName)
        iconPack.drawableFor(componentName = key)
            ?: getSystemTypeIcon(appWithComponent = appWithComponent)
    }
}